...
public class Car {
    ...
    public void startUp() {
        if(this.isOn) {          // 시동이 걸려 있으면
            ...
            this.isOn = true;    // isOn 앞에는 this. 이 있는 것이지만 this. 을 생략해도 자동으로 적힌다.
            System.out.println("시동을 걸었습니다. 부릉부릉~");
        }
    }

    /* 설명. 시동이 걸린 상태인 경우에만 호출 시 마다 10km/h 씩 속도를 증가시킨다. */
    public void go() {
        if(isOn) {          // 시동이 켜져 있어 달릴 수 있음
            System.out.println("차가 앞으로 움직입니다.");
            speed += 10;
            System.out.println("현재 차의 속도는 " + speed + "km/h 입니다.");
        } else {            // 시동이 꺼져 있어 달릴 수 없음
            System.out.println("차의 시동이 걸려있지 않습니다. 시동을 먼저 걸어주세요.");
        }
    }

    /* 설명. 시동이 걸려 있고 달리는 상태인 경우에만 멈출 수 있다. */
    public void stop() {
        if(isOn) {
            if(speed > 0) {
                speed = 0;
    ...
    /* 설명. 시동이 걸려 있는 상태인 경우 시동을 끄고, 이미 꺼진 상태라면 끌 수 있고 주행중이라면 끌 수 없다. */
    public void turnOff() {
        if(isOn) {
            if(speed > 0) {
