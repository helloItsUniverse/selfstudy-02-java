...
public class Application {
    public static void main(String[] args) {
        ...
        * */
        ...
        int num = 20;
        System.out.println("call by value 이전: " + num);
        pt.testPrimitiveTypeParameter(num);                                          // call by value(리터럴 값에 의한 호출)
        System.out.println("call by value 이후: " + num);                             // 영향받는 것을 확인할 수 있음

        /* 목차. 2. 기본자료형 배열을 매개변수로 전달 받는 메소드 호출 */
        int[] iArr = new int[]{1, 2, 3, 4, 5};
        System.out.println("call by reference 이전: " + Arrays.toString(iArr));
        pt.testPrimitiveTypeArrayParameter(iArr);                                    // call by reference(참조값에 의한 호출)
        System.out.println("call by reference 이후: " + Arrays.toString(iArr));       // 영향받는 것을 확인할 수 있음
