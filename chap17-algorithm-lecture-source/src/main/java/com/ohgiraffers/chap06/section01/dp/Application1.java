...
public class Application1 {
    ...
    public static int solution(int N) throws Exception {
        ...
        /* 설명.
         *  - 5kg까지는 앞의 if문에서 처리했기 때문에 6kg부터(5kg 이하는 1이 측정되어야 하므로 +1을 함)
         *    dp배열에 저장한다.(3kg과 5kg에 대한 개념을 해당 DP애 저장한다.)
         *  - 6번째 index까지 초기값을 설정해 주어야 정상적으로 동작함
         *  - 점화식에 dp[1 - 5]가 사용되기 때문
        * */
        for (int i = 6; i <= N; i++) {

            /* 설명. 한 종류의 봉지로만 성탕을 처리하는 경우 */
            if (i % 5 == 0) {
                dp[i] = dp[i - 5] + 1;
            } else if (i % 3 == 0) {
                dp[i] = dp[i - 3] + 1;
                ...
                /* 설명. 3 또는 5의 배수 구간이 아니지만 3kg 또는 5kg짜리 봉지를 하나 더 들고 갈 수 있는 경우라면(ex. 23kg)*/
