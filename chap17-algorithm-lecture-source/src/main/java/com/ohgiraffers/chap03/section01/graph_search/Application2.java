...
/* 수업목표. BFS와 x, y 좌표를 활용한 문제를 해결할 수 있다.(feat. 배열의 인ㄷ게스와 반대 개념) */
/* 필기.
...
public class Application2 {
    /* 설명. 테스트 코드에서 입력받는 방식을 문제에서 요구하는 대로 작성하고 문자열로 입력받기 위해 변경된 코드 */
    ...
    /* 설명. 상하좌우개념의 좌표 배열들 */
    static int dirX[] = {0, 0, -1, 1};      // 하, 상, 좌, 우 방향벡터
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < M; j++) {
                if(visit[i][j] == false && map[i][j] == 1) {
                    count++;
                    bfs(j, i);          // bfs의 인자는 x좌표, y좌표 순으로 받을 것이므로 i와 j를 활딘하고 넘겨준다.
                }
            }
        }
